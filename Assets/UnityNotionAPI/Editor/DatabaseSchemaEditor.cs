...
namespace BennyKok.NotionAPI.Editor
{
    ...
    public class DatabaseSchemaEditor : UnityEditor.Editor
    {
        ...
        public void CreateSerializedDatabaseFile(DatabaseSchema target)
        {
            var sb = new StringBuilder();
            var indentLevel = 0;
            var className = RemoveWhitespace(target.name) + "Database";
        ...
        private static void OnScriptsReload()
        {
            ...
            var className = EditorPrefs.GetString("NotionAPI_DatabaseName");
            var path = EditorPrefs.GetString("NotionAPI_DatabasePath");
            var schemaInstanceId = EditorPrefs.GetInt("NotionAPI_DatabaseSchemaId");
